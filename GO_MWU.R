...
dir <- "C:/Users/User/OneDrive/Documents/OregonStateUniversity/Coral_Projects/GreatBarrierReef/Pdae_Project/AF_Paper/ResponseToEditors/GO_MWU/Carlys_GOMWU"
setwd(dir)

# Edit these to match your data file names: 
input="TransToLogP_NegLog_23.csv" # two columns of comma-separated values: gene id, continuous measure of significance. The adjP value was attained by negative log of the p-value as in the manual for GO_MWU. To perform standard GO enrichment analysis based on Fisher's exact test, use binary measure (0 or 1, i.e., either sgnificant or not).
goAnnotations="GOannot_MWU.tab" # two-column, tab-delimited, one line per gene, multiple GO terms separated by semicolon. If you have multiple lines per gene, use nrify_GOtable.pl prior to running this script.
...
# Having a seed of 16 in Bowtie does not change GO functional groups
# Having a seed of 14 in BOwtie does not change GO functional groups

# Plotting results
quartz()
results=gomwuPlot(input,goAnnotations,goDivision,
#	absValue=-log(0.05,10),  # genes with the measure value exceeding this will be counted as "good genes". 
	absValue=1, #Specify absValue=1 for Mann Whitney OR 0.001 if you are doing Fisher's exact test for standard GO enrichment or analyzing a WGCNA module (all non-zero genes = "good genes").
	level1=0.1, # FDR threshold for plotting. Specify level1=1 to plot all GO categories containing genes exceeding the absValue.
	level2=0.01, # FDR cutoff to print in regular (not italic) font.
	level3=0.001, # FDR cutoff to print in large bold font.
	txtsize=3.5,    # decrease to fit more on one page, or increase (after rescaling the plot so the tree fits the text) for better "word cloud" effect
...
#write a table using the Part of the Bowtie mapping document and the 
